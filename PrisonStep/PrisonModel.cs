...
namespace PrisonStep
{
    ...
    public class PrisonModel
    {
        ...
        private class Door
        {
            ...
            private List<Pie> piesStuck = new List<Pie>();
            public List<Pie> Pies { get { return piesStuck; } }
            public void AddPie(Pie pie) { piesStuck.Add(pie); }
        }
        ...
        public PrisonModel(PrisonGame game, int section)
        {
            ...
            lineDraw = new PSLineDraw(game, game.Camera);
            game.Components.Add(lineDraw);
        }
        ...
        public void AttachPie(Pie pie, char DoorNum)
        {
            for (int b = 0; b < model.Bones.Count; b++)
            {
                ...
                {
                    // What is the door number?
                    char dnum = model.Bones[b].Name[9];
                    if (dnum == DoorNum)
                    {
                        doors[int.Parse("" + dnum)].AddPie(pie);
                    }
                }
        ...
        public float DoorPercentOpen(int dnum)
        {
            Door door;
            if (doors.TryGetValue(dnum, out door))
            {
                return door.Y / DoorMaxY;
            }

            return 0;
        }

        public float DoorHeight(int dnum)
        {
            Door door;
            if (doors.TryGetValue(dnum, out door))
            {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            foreach (Door door in doors.Values)
            {
                ...
                float changeInY = door.Y - prevDoorY;
                if (changeInY != 0)
                {
                    foreach (Pie pie in door.Pies)
                    {
                        pie.AddDoorMovement(changeInY);
                    }
                }
        ...
        public void Draw(GraphicsDeviceManager graphics, GameTime gameTime)
        {
            ...
            DrawModel(graphics, model, Matrix.Identity, gameTime);
        }
        ...
        private void DrawModel(GraphicsDeviceManager graphics, Model model, Matrix world, GameTime gameTime)
        {
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                ...
                mesh.Draw();
                //lineDraw.Crosshair((absoTransforms[mesh.ParentBone.Index] * world).Translation, 20, Color.Green);
