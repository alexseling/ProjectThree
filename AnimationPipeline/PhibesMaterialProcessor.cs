...
-1801,        0,  13.1765,      80,   1406,    181,  -1801,       0, 13.1765,  80};
        ...
        public override MaterialContent Process(MaterialContent input, ContentProcessorContext context)
        {
            ...
            else if (roomLit)
            {
                ...
                if (basicMaterial.Texture == null)
                {
                    ...
                    string effectFile = Path.GetFullPath("RoomLights1.fx");
                    customMaterial.Effect = new ExternalReference<EffectContent>(effectFile);
                    customMaterial.OpaqueData.Add("DiffuseColor", basicMaterial.DiffuseColor);
                }
                else
                {
                    string effectFile = Path.GetFullPath("RoomLights2.fx");
                    customMaterial.Effect = new ExternalReference<EffectContent>(effectFile);
            ...
            {
                for (int sectionNum = 1; sectionNum <= 6; sectionNum++)
                {
                    for (int lightNum = 1; lightNum <= 3; lightNum++)
                    {
                        string beginning = "S" + sectionNum + "_Light" + lightNum;
                        customMaterial.OpaqueData.Add(beginning + "Location", LightInfo(sectionNum, 2 * lightNum - 2));
                        customMaterial.OpaqueData.Add(beginning + "Color", LightInfo(sectionNum, 2 * lightNum - 1));
                    }
                }
            }
        ...
        public bool RoomLit { get { return roomLit; } set { roomLit = value; } }
    }
}
